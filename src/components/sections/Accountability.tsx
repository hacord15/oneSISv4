        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="mt-4 font-display text-[2rem] leading-[1.12] text-[var(--color-ink)] sm:text-[2.5rem]">
              {accountability.heading} <span className="accent">{accountability.headingAccent}</span>
              <br />
      ...
      <div className="border-t border-[var(--color-border)] bg-white">
        <Container className="px-0 sm:px-8 lg:px-16">
          <div className="grid grid-cols-1 divide-y divide-[var(--color-border)] sm:grid-cols-3 sm:divide-x sm:divide-y-0">
            {accountability.items.slice(0, 3).map((item) => (
          ...
          <div className="grid grid-cols-1 divide-y divide-[var(--color-border)] border-t border-[var(--color-border)] sm:grid-cols-3 sm:divide-x sm:divide-y-0">
            {accountability.items.slice(3, 6).map((item) => (
              <div key={item.index} className="px-6 py-10 sm:px-8">
                <span className="font-display text-4xl text-[var(--color-muted-2)]">
                  {item.index}
                </span>
